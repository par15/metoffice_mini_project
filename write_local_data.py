...
## only considers a single grid point
...
for d in range(n_days):
	temp,pres,humidity,wind = ensemble_stats.calculate_mean(lat,lon,2014,month,d+1,3,ensemble_size)
...
file_ops.write_mean_netCDF(lat,lon,year,month,n_days,temp_mean,pres_mean,humidity_mean,wind_mean)

# write test_data to file

test_size = 12 - ensemble_size

file_ops.write_test_data(lat,lon,year,month,n_days,hour,test_size)
